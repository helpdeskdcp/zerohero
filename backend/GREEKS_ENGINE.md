...
| item | |
|---|---|
| **deploy** | the router + per-cycle engine call go live on the next `oi-dashboard` restart — **held to a market-closed window** (a mid-session restart re-triggers the NIFTY aggregator re-seed blind period). Until then: `python -m app.greeks_engine --once` computes on demand; the one row above is real. |
