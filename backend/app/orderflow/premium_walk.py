...
def rewalk_leg(opt_map: dict, *, entry_price: float, side: str,
               entry_ts: Optional[str], exit_ts: Optional[str]) -> Optional[dict]:
    ...
    # premium path between entry and exit (inclusive) -> MFE / MAE
    lo = bisect_right(series, (entry_ts, -1.0))
    hi = bisect_right(series, (exit_ts, float("inf")))
    path = [v for _, v in series[lo:hi]] or [p_entry, p_exit]
