...
# relative future expiries — a fixed date would expire and flip AUTO to the next one
_E1 = (datetime.now(timezone.utc) + timedelta(days=7)).strftime("%d%b%Y").upper()
_E2 = (datetime.now(timezone.utc) + timedelta(days=14)).strftime("%d%b%Y").upper()
...
def test_standalone_resolver_uses_valid_master_and_expiry(monkeypatch, tmp_path):
    ...
    c._master = [
        {"exch_seg":"NFO","name":"BANKNIFTY","symbol":f"BANKNIFTY{_E1}50000CE","token":"1","instrumenttype":"OPTIDX","expiry":_E1,"strike":"5000000","lotsize":"15"},
        {"exch_seg":"NFO","name":"BANKNIFTY","symbol":f"BANKNIFTY{_E2}50000CE","token":"2","instrumenttype":"OPTIDX","expiry":_E2,"strike":"5000000","lotsize":"15"},
    ]
    r = c.resolve_option_contract("BANKNIFTY", "AUTO", "ATM", "CE", spot=50000)
    assert r["status"] == "OK" and r["underlying"] == "BANKNIFTY" and r["token"] == "1"
    assert r["available_expiries"] == [_E1, _E2]
...
def test_auto_roll_skips_the_0dte_contract_on_expiry_day(tmp_path):
    ...
    next_wk = (now + timedelta(days=7)).strftime("%d%b%Y").upper()   # relative, so it stays in the future
    c._master = [
        {"exch_seg": "NFO", "name": "NIFTY", "symbol": f"NIFTY{today}24000CE", "token": "1",
         "instrumenttype": "OPTIDX", "expiry": today, "strike": "2400000", "lotsize": "65"},
        {"exch_seg": "NFO", "name": "NIFTY", "symbol": f"NIFTY{next_wk}240000CE", "token": "2",
         "instrumenttype": "OPTIDX", "expiry": next_wk, "strike": "2400000", "lotsize": "65"},
    ]
    ...
    assert c.resolve_option_contract("NIFTY", "AUTO_ROLL", "ATM", "CE", spot=24000)["expiry"] == next_wk
